var home = Vue.component("home", {
    ...
    <div v-if="populate.active" class="populate-progress" style="
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:5px;">
            <span>Generating local entries with Claude Opus for {{ populate.label }}…<span v-if="populate.total"> (day {{ populate.done }}/{{ populate.total }})</span></span>
    ...
    methods: {
        ...
        saveWeek() {
            ...
            if (!confirm(
                "Push " + drafts.length + " draft entr" + (drafts.length === 1 ? "y" : "ies") +
                " this week to Toggl? Drafts without a project are skipped."
            )) {
                return;
            }
            this.$store.commit('setLoading', {module_name: 'saveweek', loading: true});
            axios.post("saveweek", {
                from: this.$store.getters.getFrom,
                to: this.$store.getters.getTo,
            }).then(response => {
                console.log("saveweek", response);
                const d = response.data || {};
                if (d.skipped) {
                    alert("Queued " + d.queued + " for Toggl. Skipped " + d.skipped +
                          " draft(s) with no project assigned.");
                }
                this.$store.commit('setLoading', {module_name: 'saveweek', loading: false});
                // The sync worker pushes in the background; refetch shortly so
                // synced entries lose their draft styling once Toggl confirms.
                setTimeout(() => this.fetchModule("toggl", 0), 3000);
            }).catch((err) => {
                this.$store.commit('setLoading', {module_name: 'saveweek', loading: false});
                alert("Save week failed: " + (err.message || "unknown error"));
            });
        },
        populateLocal() {
        ...
        populateDay(req) {
            ...
            const entries = count + " existing draft entr" +
                (count === 1 ? "y" : "ies");
            if (!confirm(
                "Replace " + entries + " on " + req.day +
                " with entries generated by Claude? Existing edited drafts " +
                "for this day will also be replaced."
            )) {
                return;
            }
